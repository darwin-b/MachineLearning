...
for dataset in datasets:
    ...
    n_runs = 0
    LikelihoodArray = np.zeros((1, max_runs))
    while (n_runs - max_runs<0):

        for k_valid in range(0,k):
            print("Run : ", n_runs + 1, " K=", k_valid)

            # ParameterArray = findingParameters(XTrainKBags[k_valid])
            p_array = np.zeros((2, n_features))
            p_array[1] = (x_train.sum(axis=0) + 1) / (n_size + 2)
            p_array[0] = 1 - p_array[1]

            # InfomationMatrix, probDict = calculateWeights(XTrainKBags[k_valid], ParameterArray)
            ...
            # PredictProbabalitiesTestData(XTest, ParameterArray, parents, XTrainKBags[k_valid], probDict, predictions,
            #                              clusterProbabalities[0, k_valid])
